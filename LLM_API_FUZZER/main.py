...
def create_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        '--max_token_threshold',
        type=int,
        help='Maximum amount of tokens LLM should not bypass when producing output'
    )
    parser.add_argument(
        '--rate_limit_test_count',
        type=int,
        help='Number of requests for rate limit testing'
    )
    parser.add_argument(
        '--rate_limit_interval',
        type=float,
        help='Time between requests for rate limit testing (seconds)'
    )
    return parser
