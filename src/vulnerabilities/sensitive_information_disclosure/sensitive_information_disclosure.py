...
class SensitiveInformationDisclosureFuzzer:
    def __init__(
        self, 
        model_endpoint, 
        request_handler, 
        ...
        default_tests_path = os.path.join(base_dir, './data/sensitive_information_disclosure_payloads.json')
    ...
    def fuzz(self) -> Dict:
        ...
        for test_case in self.tests:
            try:
                # Extract prompt from test case and replace placeholder with PII_Name
                prompt = test_case["prompt"].replace("{placeholder}", self.PII_Name)
                ...
                # Create a modified test case with the replaced prompt for analysis
                modified_test_case = test_case.copy()
                modified_test_case["prompt"] = prompt
                
                # Analyze the response
                result_entry = self.analyze_response(
                    test_case=modified_test_case,
                    response=response
                )
                ...
                result_entry.update({
                    ...
                    'original_prompt': original_prompt  # Store the original prompt with placeholder
                })
    ...
    def display_results(self, results: Dict[str, Any], response_field: str = None) -> None:
        ...
        """
        print("\n" + "="*80)
        print(f"{Fore.CYAN}SENSITIVE INFORMATION DISCLOSURE FUZZING RESULTS{Style.RESET_ALL}")
        print("="*80)
